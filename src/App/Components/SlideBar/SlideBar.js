    ...
    <aside className="w-60 -translate-x-48 fixed transition transform ease-in-out duration-1000 z-50 flex h-screen bg-[#1E293B] ">
      ...
      <div className="mini mt-20 flex flex-col space-y-2 w-full h-[calc(100vh)]">
        {props.data.map((menu) => (
          ...
          >
            {menu.tag && props.count > 0 && <div className="bg-red-500 text-white absolute rounded-full h-5 w-5 right-1 top-1 flex justify-center items-center font-bold text-xs">{props.count}</div>}
